...
DWORD  SClient::RecvThread(void* pParam)
{
	...
	while(pClient->m_bConning)				//连接状态
	{
		...
		if (reVal > 0)
		{
            ...
            {
                cout<<"IP: "<<pClientIP<<"\t向全体客户端发送数据:"<<temp+3<<endl;
                ToAllCliBuf(temp, pClientIP);
            }
            else
            {
...
DWORD SClient::SendThread(void* pParam)
{
	...
	while(pClient->m_bConning)//连接状态
	{
        ...
        {
			//发送数据
			int val = send(pClient->m_socket, pClient->BufSend, strlen(pClient->BufSend),0);
			//处理返回错误
			if (val==SOCKET_ERROR)
			{
				int nErrCode=WSAGetLastError();
			...
			memset(pClient->BufSend, 0, MAX_NUM_BUF);
		}
	}
